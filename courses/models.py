...
class Course(models.Model):
    ...
    code = models.CharField('course code', max_length=10,
                            help_text=_('Course code (max 10 characters)'))
    description = models.CharField(max_length=400, help_text=_(
        'Course description (max 400 characters)'))
    ...
    enable_gamification = models.BooleanField('gamification',
                                              default=True,
    ...
    def clean(self):
        ...
        if self.show_scoreboard and not self.enable_gamification:
            raise ValidationError(
                _('Scoreboard requires gamification to be enabled'))
        if self.show_leaderboard and not self.enable_gamification:
            raise ValidationError(
                _('Leaderboard requires gamification to be enabled'))
